...
render() {
    sed -e "s|/usr/lib/silverblue|$(esc "$LIB_DIR")|g" \
        ...
        -e "s|silverblue-boot-check|$(esc "$UNIT_PREFIX")-boot-check|g" \
...
main() {
    ...
    # it, and rollback.sh that exec's it) with no runtime config plumbing. Each baked line only
    # assigns when the variable is not already in the environment, so runtime overrides (e.g.
    # SB_TRIES=1 in the QEMU harness, or the installer pointing SB_TOPLEVEL_MNT/SB_EFI_DIR at
    # its own mounts for --generate-manifest) keep working; the engine's own
    # ': "${SB_X:=default}"' lines run afterwards and keep these baked values. printf %q keeps
    # the values safe to re-source.
    local eng="$A/usr/bin/${BIN_PREFIX}-update"
    # shellcheck disable=SC2016  # the ${%s:-} is a printf template, expansion happens at runtime
    bake() { printf '[[ -n "${%s:-}" ]] || %s=%q\n' "$1" "$1" "$2"; }
    {
        ...
        bake SB_LIB_DIR      "$LIB_DIR"
        bake SB_VERIFY_UNIT  "/usr/lib/systemd/system/${UNIT_PREFIX}-mark-good.service"
        bake SB_TOPLEVEL_MNT "$TOPLEVEL_MNT"
        bake SB_EFI_DIR      "$EFI_DIR"
        bake SB_KEEP         "$KEEP_SNAPSHOTS"
        bake SB_TRIES        "$BOOT_TRIES"
        bake SB_DISTRO_NAME  "$DISTRO_NAME"
        bake SB_ESP_SUBDIR   "$ESP_SUBDIR"
        bake SB_SORT_KEY     "$SORT_KEY"
        bake SB_STATE_SUBDIR ".${UNIT_PREFIX}"
        bake SB_MANIFEST_PATHS     "${MANIFEST_PATHS:-/usr /boot}"
        bake SB_VERIFY_ON_ROLLBACK "${VERIFY_ON_ROLLBACK:-warn}"
        tail -n +2 "$REPO/src/update-engine/silverblue-update"
    } > "$eng"
    chmod 0755 "$eng"

    # Bootloader + integrity helpers: installed verbatim (they read the engine-provided SB_*
    # vars at runtime).
    ...
    install -Dm0644 "$REPO/src/update-engine/integrity.sh"   "$A${LIB_DIR}/integrity.sh"
    ...
    render < "$REPO/src/init/init-backends.sh"        > "$A${LIB_DIR}/init-backends.sh"
    render < "$REPO/src/init/silverblue-boot-check.sh" > "$A${LIB_DIR}/${UNIT_PREFIX}-boot-check.sh"
    chmod 0755 "$A${LIB_DIR}/${UNIT_PREFIX}-mark-good.sh" "$A${LIB_DIR}/${UNIT_PREFIX}-rollback.sh" \
        "$A${LIB_DIR}/${UNIT_PREFIX}-boot-check.sh"
    ...
    # OpenRC/dinit integrations: shipped as installer payload only (the live ISO itself always
    # runs systemd); install-lib.sh copies the right one onto the target per INIT_SYSTEM.
    render < "$REPO/src/init/openrc/silverblue-mark-good" \
        | install -Dm0755 /dev/stdin "$A/usr/local/share/distro/init/openrc/${UNIT_PREFIX}-mark-good"
    render < "$REPO/src/init/dinit/silverblue-mark-good" \
        | install -Dm0644 /dev/stdin "$A/usr/local/share/distro/init/dinit/${UNIT_PREFIX}-mark-good"
    ...
    local fp; fp=$(mktemp)
    {
        ...
        printf '  ["%s/%s-boot-check.sh"]="0:0:755"\n'  "$LIB_DIR" "$UNIT_PREFIX"
